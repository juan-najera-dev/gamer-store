...
db.post("/", (request, response) => {
  ...
  const model = new Accessory({
    partNumber: body.partNumber,
    title: body.title,
    manufacturer: body.manufacturer,
    technology: body.technology,
    conectivity: body.conectivity,
    price: body.price,
    img: body.img,
  });
...
db.put("/:id", (request, response) => {
  ...
  Accessory.findByIdAndUpdate(request.params.id, updatedModel)
    .then((model) => {
...
db.delete("/:id", (request, response) => {
  Accessory.findByIdAndDelete(request.params.id)
    .then((result) => {
      if (result === null) {
        response
          .status(404)

...
db.post("/", (request, response) => {
  ...
  if (
    (body.partNumber && body.title && body.detail && body.price && body.img) ===
    undefined
  ) {
    return response
      .status(400)
...
db.put("/:id", (request, response) => {
  ...
  Service.findByIdAndUpdate(request.params.id, updatedModel)
    .then((model) => {
...
db.delete("/:id", (request, response) => {
  Service.findByIdAndDelete(request.params.id)
    .then((result) => {
      if (result === null) {
        response
          .status(404)
